    ...
    private function buildCodeParts()
    {
        ...
        if ($this->json_form->aftervalidation === 'send-email') {
            ...
            if (!empty($this->json_form->sentMessage)) {
                $if_posted_6[] = '            \'sent_message\'    => \'' . $this->sanitize($this->json_form->sentMessage) . '\',';
            }
        ...
        if ($this->json_form->ajax !== 'true') {
            ...
            $scripts = array(
                ...
                'var loadData = function(data, index, container, formId) {',
                '    $(container).attr(\'data-ajax-form-id\', formId);',
                ...
                '    } else {',
                '        loadedCount ++;',
                '        if (loadedCount === ajaxForms.length) {',
                '            $.holdReady(false);',
                '        }',
                '    }',
                '};',
                ...
                '        }).done(function(data) {',
                '            $.holdReady(true);',
                '            loadData(data, 0, currentForm.container, currentForm.formId);',
